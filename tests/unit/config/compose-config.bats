...
@test "compose config: scanner-init uses DHI debian-base" {
    run grep -A2 "scanner-init:" "$COMPOSE"
    assert_output --partial "dhi.io/debian-base:trixie"
}

@test "compose config: state-init uses DHI debian-base" {
