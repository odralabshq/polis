...
@test "clamav: container exists" {
    # Use strict matching to avoid matching scanner-init
    run bash -c "docker ps -a --format '{{.Names}}' | grep -x ${CLAMAV_CONTAINER}"
    assert_success
...
@test "clamav: runs as scanner user" {
    run docker exec "${CLAMAV_CONTAINER}" id -u -n
    assert_success
    assert_output "scanner"
}
...
@test "clamav: has required capabilities added back" {
    ...
    # SETGID/SETUID no longer required as we run as fixed user 100:101
...
@test "clamav: returns version info" {
    run docker exec "${ICAP_CONTAINER}" sh -c "echo 'VERSION' | nc scanner 3310"
    assert_success
...
@test "clamav: signature database is loaded" {
    ...
    # Check for either .cvd or .cld (local database)
...
@test "healthcheck: gateway can reach ICAP squidclamav" {
    run docker exec "${GATEWAY_CONTAINER}" sh -c "printf 'OPTIONS icap://sentinel:1344/squidclamav ICAP/1.0\r\nHost: sentinel\r\n\r\n' | timeout 3 nc sentinel 1344 | head -1"
    assert_success
    assert_output --partial "200"
}

@test "healthcheck: gateway can reach ICAP echo" {
    run docker exec "${GATEWAY_CONTAINER}" sh -c "printf 'OPTIONS icap://sentinel:1344/echo ICAP/1.0\r\nHost: sentinel\r\n\r\n' | timeout 3 nc sentinel 1344 | head -1"
    assert_success
    assert_output --partial "200"
}
...
@test "icap: has gosu installed" {
    # gosu is removed in favor of direct execution
    skip "gosu removed in non-root hardening"
...
@test "clamav: database volume exists" {
    run docker volume ls --filter "name=polis-scanner-db" --format '{{.Name}}'
    assert_success
    assert_output "polis-scanner-db"
}
