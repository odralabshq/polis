...
generate_dockerfile() {
    local agent="$1"
    local output="${PROJECT_ROOT}/build/workspace/Dockerfile.generated"

    cat > "$output" << HEADER
# Auto-generated by polis CLI — do not edit manually
# Regenerate with: ./polis init --agent=${agent} --local
FROM polis-workspace-oss:latest

COPY agents/${agent}/ /tmp/agents/${agent}/
RUN chmod +x /tmp/agents/${agent}/install.sh && /tmp/agents/${agent}/install.sh
RUN rm -rf /tmp/agents

ENTRYPOINT ["/sbin/init"]
HEADER

    log_success "Generated Dockerfile for agent: ${agent}"
}
