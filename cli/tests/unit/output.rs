...
fn test_styles_default_has_no_colors() {
    ...
    let text = "test";
    let result = text.style(styles.success);
...
fn test_styles_colorize_applies_colors() {
    ...
    styles.colorize();
    let result = format!("{}", "test".style(styles.success));
    assert!(result.contains("\x1b["), "should contain ANSI escape code");
    assert!(result.contains("32"), "should contain green color code");
}
