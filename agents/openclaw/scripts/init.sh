...
# --- Intercept: onboard ---
# Native `openclaw onboard` overwrites openclaw.json and tries systemctl,
# both of which break polis-managed config. We replace it with a direct
# auth-profiles.json + .env writer.
if [[ "${1:-}" == "onboard" ]]; then
    CONFIG_DIR="/home/polis/.openclaw"
    AUTH_FILE="${CONFIG_DIR}/agents/default/agent/auth-profiles.json"
    ENV_FILE="${CONFIG_DIR}/.env"
    mkdir -p "$(dirname "$AUTH_FILE")"

    echo ""
    echo "=== Polis Agent Key Setup ==="
    echo ""
    echo "Supported providers: anthropic, openai, google, openrouter"
    echo ""
    read -rp "Provider (anthropic/openai/google/openrouter): " provider
    provider=$(echo "$provider" | tr '[:upper:]' '[:lower:]')
    case "$provider" in
        anthropic|openai|google|openrouter) ;;
        gemini) provider="google" ;;
        *)
            echo "ERROR: Unknown provider '$provider'. Use: anthropic, openai, google, or openrouter" >&2
            exit 1
            ;;
    esac
    ...
    existing=$(cat "$AUTH_FILE" 2>/dev/null || echo "{}")
    if ! echo "$existing" | jq . >/dev/null 2>&1; then
        existing="{}"
    fi
    updated=$(echo "$existing" | jq --arg p "$provider" --arg k "$api_key" \
        '. + {($p): {"apiKey": $k}}')
    echo "$updated" > "$AUTH_FILE"
    chmod 600 "$AUTH_FILE"

    # Update .env so key persists across restarts
    env_var=""
    case "$provider" in
    ...
    # Update model in openclaw.json if config exists
    CONFIG_FILE="${CONFIG_DIR}/openclaw.json"
    if [[ -f "$CONFIG_FILE" ]] && command -v jq &>/dev/null; then
        model=""
        case "$provider" in
            anthropic)  model="anthropic/claude-sonnet-4-20250514" ;;
            openai)     model="openai/gpt-5.1-codex-mini" ;;
            google)     model="google/gemini-2.5-pro" ;;
            openrouter) model="openrouter/anthropic/claude-sonnet-4-20250514" ;;
        esac
        if [[ -n "$model" ]]; then
            jq --arg m "$model" '.agents.defaults.model.primary = $m' \
